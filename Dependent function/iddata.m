...
%   $Revision: 1.20.4.7 $  $Date: 2007/12/14 14:43:46 $
...
if ni && isa(varargin{1},'iddata')
    ...
    if ni==2 % forgiving syntax  dat = iddata(y,u) with y and u
        ...
        dat = varargin{1};
        if ni>1,
            error('ident:general:useSetForProp',...
             'Use "set" command to modify the properties of %s objects.','IDDATA');
...
% Note: version was string '1.0' before R20078a and '0.1' in first version (R13?)
...
if DoubleInputs > 0
    ...
    if ~isempty(error_str.message), error(error_str), end
        ...
        if DoubleInputs > 2
            ...
            for kk = 1:length(Value)
                if length(Value{kk})>1 || Value{kk}<0 % Check also the others
                    error('ident:iddata:nonPositiveTs',...
                        'For time domain data, the sampling interval ("Ts") must be [] or a positive scalar for each experiment.')
                end
            end
            dat.Ts = Value;
            varargin = varargin(2:end);
...
for kk=1:length(Ts)
    if strcmp(dat.Domain,'Time')
        ...
        if isempty(Ts{kk})
            error('ident:iddata:freDataTsRequired',...
                ['For frequency domain data, a sampling interval ("Ts") must always be supplied.',...
                '\nUse Ts = 0 for continuous time data.'])
        end
    end
end
